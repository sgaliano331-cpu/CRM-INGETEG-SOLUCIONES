...
export default function Liquidacion() {
  ...
  const [newTarifa, setNewTarifa] = useState({ tipo: 'equipo', nombre: '', valor_tecnico: '', tipo_servicio: 'Mantenimiento' });
  ...
  const guardarTarifa = async (e) => {
    ...
    try {
      ...
      setNewTarifa({ tipo: 'equipo', nombre: '', valor_tecnico: '', tipo_servicio: 'Mantenimiento' });
      setComboEquipos([]);
      fetchTarifas();
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    ...
                    <th className="text-left py-2 px-4 text-xs font-semibold text-slate-500">Servicio</th>
                ...
                <tbody>
                  {tarifas.filter(t => t.tipo === 'equipo').map(t => (
                    <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className="py-2 px-4">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${t.tipo_servicio === 'Reparación' ? 'bg-orange-100 text-orange-700' : t.tipo_servicio === 'Garantía' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                          {t.tipo_servicio || 'Mantenimiento'}
                        </span>
                      </td>
                  ...
                  {tarifas.filter(t => t.tipo === 'equipo').length === 0 && (
                    <tr><td colSpan={4} className="py-6 text-center text-sm text-slate-400">No hay tarifas de equipo configuradas</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                  ...
                  <tbody>
                    {informe.servicios.map((s, i) => (
                      <tr key={s.id} className={`border-b border-slate-100 hover:bg-slate-50 ${s.liquidado ? 'opacity-50 bg-green-50/30' : ''}`}>
                        ...
                        <td className="py-2.5 px-3">
                          <span className={`text-xs px-1.5 py-0.5 rounded-full ${s.tipo_servicio === 'Reparación' ? 'bg-orange-100 text-orange-700' : s.tipo_servicio === 'Garantía' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
