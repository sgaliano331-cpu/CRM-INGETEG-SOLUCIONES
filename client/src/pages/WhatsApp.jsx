...
export default function WhatsApp() {
  ...
  const enviarMasivoExcel = async (e) => {
    ...
    const extraFields = ['telefono2', 'segundo_telefono', 'segundo_numero', 'direccion', 'municipio', 'ciudad', 'barrio', 'proxima_certificacion', 'proxima certificacion', 'certificacion'];
    ...
    try {
      const contactos = masivo.excelData.map(row => {
        ...
        for (const c of extraCols) {
          ...
          else if (cl.includes('direccion')) extra.direccion = String(row[c] ?? '').trim();
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-xs font-medium text-slate-600">Segundo telefono</span>
                      </div>
                      <input
                        type="text"
                        value={contactoInfo.telefono2 || ''}
                        onChange={e => setContactoInfo(c => ({ ...c, telefono2: e.target.value }))}
                        onBlur={e => updateContacto('telefono2', e.target.value)}
                        placeholder="Agregar segundo numero..."
                        className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs text-slate-700 focus:ring-1 focus:ring-green-500 focus:border-green-500"
                      />
                    </div>
                    ...
                    {selected?.ultimo_mensaje && (
                      <div>
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span className="text-xs font-medium text-slate-600">Fecha de ultimo mensaje</span>
                        </div>
                        <p className="text-xs text-slate-700 px-2.5 py-1.5 bg-slate-50 rounded-lg">
                          {new Date(selected.ultimo_mensaje).toLocaleString('es-CO', { weekday: 'short', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true })}
                        </p>
                      </div>
                    )}
